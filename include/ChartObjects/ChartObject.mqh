...
//--- write start marker - 0xFFFFFFFFFFFFFFFF
   if(FileWriteLong(file_handle,-1)!=sizeof(long))
      return(false);
//--- write object type
   ENUM_OBJECT object = (ENUM_OBJECT)Type();
   if(FileWriteInteger(file_handle,object,INT_VALUE)!=INT_VALUE)
      return(false);
//--- write object name
...
//--- read and check object type
   ENUM_OBJECT object = (ENUM_OBJECT)FileReadInteger(file_handle,INT_VALUE);
   if(object!=Type())
      return(false);
//--- read object name
...
//--- read number of levels
   num=FileReadInteger(file_handle,INT_VALUE);
   //long whatisthis = FileReadLong(file_handle);
   if(num != 0 && !ObjectSetInteger(m_chart_id,m_name,OBJPROP_LEVELS,0,num))
      return(false);
//--- read levels
   if(num!=0)
     {
